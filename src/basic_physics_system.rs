...
impl BasicPhysicsSystem {
    pub fn update(mut query: Query<(&mut TransformComponent, &VelocityComponent)>) {
        ...
        for (mut transform, velocity) in query.iter_mut() {
            ...
            transform.position = Self::apply_translation(
                &transform.position,
            ...
            if angular_velocity_magnitude > 0.0 {
                transform.rotation = Self::apply_rotation(
                    &transform.rotation,
                    &velocity.angular,
                    delta_time,
                );
...
mod tests {
    ...
    fn test_apply_translation() {
        ...
        let newer_position = BasicPhysicsSystem::apply_translation(&new_position, &velocity, DELTA_TIME);
    ...
    fn test_apply_rotation_x() {
        ...
        let new_rotation = BasicPhysicsSystem::apply_rotation(&rotation, &angular_velocity, DELTA_TIME);

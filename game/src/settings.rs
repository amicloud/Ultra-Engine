...
mod tests {
    ...
    fn save_file_write_permissions() {
        ...
        #[cfg(unix)]
        {
            ...
            let no_write_dir = config_dir.join("Ultra").join("no_write_dir");
...
use_https = false
    ...
    fn proper_error_propagation() {
        ...
        let non_existent_path = config_dir.join("Ultra").join("non_existent.toml");
        let result = Settings::load_from_file(&non_existent_path);
        assert!(matches!(result, Err(SettingsError::Io(_))));

        // Attempt to deserialize invalid TOML
        let invalid_toml = "invalid = toml:::";
        let invalid_path = config_dir.join("Ultra").join("invalid.toml");
        ...
        let invalid_save_path = config_dir.join("Ultra").join("settings");
